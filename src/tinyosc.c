...
static uint32_t tosc_vwrite(char *buffer, const int len,
  ...
  if (address == NULL || ((int)i) >= len) return -1;
  tosc_strncpy(buffer, address, len);
  i = (i + 4) & ~0x3;
  buffer[i++] = ',';
  int s_len = (int) strlen(format);
  if (format == NULL || (((int)i) + s_len) >= len) return -2;
  ...
  for (int j = 0; format[j] != '\0'; ++j) {
    switch (format[j]) {
      case 'b': {
        ...
        if (((int)i) + 4 + (int)n > (int)len) return -3;
